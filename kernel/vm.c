...
// Copy the newly allocated pages to the kernel page table.
uint64
...
// free_leaf_pte = 0: All leaf mappings must already have been removed.
// free_leaf_pte = 1: Release all memory except leaf nodes.
void
freewalk(pagetable_t pagetable, int free_leaf_pte)
{
  ...
  for(int i = 0; i < 512; i++){
    ...
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
      ...
      freewalk((pagetable_t)child, free_leaf_pte);
      pagetable[i] = 0;
    } else if(pte & PTE_V){
      if (free_leaf_pte)
        pagetable[i] = 0;
...
// Do not allocate leaf physical memory.
int
uvmkcopy(pagetable_t user, pagetable_t kernel, uint64 start, uint64 end) {
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = start; i < end; i += PGSIZE){
    ...
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
    if (mappages(kernel, i, PGSIZE, pa, flags & (~PTE_U)) < 0) {
...
void vmprint_helper(int index, pte_t pte, int level) {
  if (!(pte & PTE_V))
    return;
  ...
  for (int i = 0; i < 512; i++) {
    vmprint_helper(i, child[i], level - 1);
  }
}
...
void vmprint(pagetable_t pagetable) {
  printf("page table %p\n", pagetable);
  for (int i = 0; i < 512; i++) {
    vmprint_helper(i, pagetable[i], 2);
  }
}
